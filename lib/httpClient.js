...
var update_cookies = function (cks, multiple) {
    if (cks) {
        all_cookies = union(all_cookies, multiple ? cks.split(' ') : cks);
    }
}
...
var post = function (options, data, callback) {
    ...
    var postData = options.headers['Content-Type'] ? JSON.stringify(data) : querystring.stringify(data);
