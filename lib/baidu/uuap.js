...
function _login(options, callback) {
    ...
    if (options.rsa) {
        ...
        var arr = [options.lt, timestamp, options.password];

        const encrypt = new JSEncrypt();
        encrypt.setPublicKey(options.rsa);
        var encryptedPassword = encrypt.encrypt(arr.join(','));

        form.password = encryptedPassword;
    }

    var tmp = new URL(options.server || options.uuapServer || UUAP_SERVER);
