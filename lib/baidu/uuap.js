...
const baiduUuap = module.exports = function (params, cb) {
    ...
    const {server, uuapServer, service} = self._options;
    serverUrl = server || `${uuapServer}?service=${service}` || UUAP_SERVER;
    const METHOD = '_login';
...
function _login(options, loginOptions, callback) {
    const {username, password, server, uuapServer} = options;
    ...
    client.post({
        ...
        method: 'POST',
        headers: {
            'Referer': server || uuapServer || UUAP_SERVER,
        }
