...
const baiduUuap = module.exports = function (params, cb) {
    ...
    const authorizeUrl = self._options.server || UUAP_SERVER;

    const METHOD = 'login';
    loginUrl = authorizeUrl.replace('authorize', METHOD);

    client.url_get(authorizeUrl, {}, function (response) { 
        if (response.statusCode === 200) {
            client.url_get(loginUrl, {
                headers: {
                    'originalUrl': authorizeUrl
                }
            }, function (res, data) {
...
function _login(options, loginOptions, callback) {
    const {username, password, server, type} = options;
    ...
    const tmpUrl = loginUrl.replace('/login', loginPath);
    ...
    client.post({
        ...
        if (code === 302 && result.redirectUrl) {
            client.get(result.redirectUrl, function (res, data) {
                const tokenMatch = data.match(/window.location.replace\("(.*)"\)/);
                const sTokenUrl = tokenMatch ? tokenMatch[1] : '';
                if (sTokenUrl) {
                    client.get(sTokenUrl, function(res, data) {
            ...
            client.get(loginUrl, function (res, data) {
                callback(client.get_cookies_string());
            });
        } else {
            callback(client.get_cookies_string());
